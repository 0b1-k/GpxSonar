...
public:
   ...
   CCartesianCoord ToCartesian(void);
   void FromCartesian(CCartesianCoord& C);
   CCartesianCoord ToSphericalCartesian(void);
...
public:
   ...
   bool ParseDegreeString(const char *szDegrees, double *dResult) {
      return ParseDegreeString(std::string(szDegrees), dResult);
   }
